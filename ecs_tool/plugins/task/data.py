...
def _fetch_latest_active_task_definition(ecs_client, name):
    ...
    if not response["taskDefinitionArns"]:
        raise EcsToolException(
            f'Unable to find active task definition with name "{name}".'
        )
...
def fetch_run_task(context: ContextObject, click_params):
    if not getattr(fetch_run_task, "task_ran", False):
        ...
        try:
            ...
            args["taskDefinition"] = _fetch_latest_active_task_definition(
                context.ecs, click_params["task_definition"]
            )
    ...
    task = context.ecs.describe_tasks(
        cluster=click_params["cluster"], tasks=[fetch_run_task.task_arn]
    )["tasks"][0]
